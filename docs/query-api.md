...
数据源：`data/processed/{DATE}/daily_summary.json`，粒度为 **日期×国家**。
...
# 指定产品 + 国家
./venv/bin/python -m scripts.query summary --days 30 --products 半开猫砂盆 --country 美国

# 指定日期范围
./venv/bin/python -m scripts.query summary --start 2026-03-01 --end 2026-03-10

# 多产品
./venv/bin/python -m scripts.query summary --days 7 --products 半开猫砂盆,欧博尔面包机 --fields 总销量,利润,TAcos
```
...
# 某产品最近3天已发货订单
./venv/bin/python -m scripts.query detail --products 欧博尔面包机 --days 3 --status Shipped

# 只取指定字段（评论引导系统用）
./venv/bin/python -m scripts.query detail --products 欧博尔面包机 --days 7 --fields 防重复编号,订单状态,ASIN
```
...
| `--products 名1,名2` | 两者 | 按产品过滤，不传则返回所有产品 |
...
站点日期、国家、货币、总销量、FBM订单、FBA订单、广告单、总销售额、
...
| 单产品分析 | `summary --days 30 --products 半开猫砂盆 --fields 总销量,总广告花费,利润,TAcos` |
| 查某笔订单 | `detail --order 114-1607332-5352268` |
| 评论引导：获取订单列表 | `detail --products 欧博尔面包机 --days 7 --status Shipped --fields 防重复编号,ASIN` |
